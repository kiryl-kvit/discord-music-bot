...
public static class FavoriteEmbedBuilder
{
    ...
    {
        return BuildAddedEmbed(item.DisplayName, item.Author, item.Duration, item.IsPlaylist, item.Alias);
    }

    public static Embed BuildAddedEmbed(string title, string? author, TimeSpan? duration,
        bool isPlaylist = false, string? alias = null)
    {
        ...
        {
            builder.AddField("Artist", DisplayConstants.AuthorOrDefault(author), inline: true);
            builder.AddField("Duration",
                DateFormatter.FormatTimeOrDefault(duration),
                inline: true);
        }

        if (alias is not null)
        {
            builder.AddField("Alias", alias, inline: true);
        }
    ...
    private static string FormatLine(FavoriteItem item, int page, int pageSize, int index)
    {
        ...
        return $"`{position}.` **{item.DisplayName}** - {DisplayConstants.AuthorOrDefault(item.Author)} `[{duration}]`";
    }
